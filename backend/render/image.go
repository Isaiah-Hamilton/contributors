...
func Image(buf *bytes.Buffer, contributors []utils.Contributor, config utils.Config) {
	...
	for i, contributor := range contributors {
		...
		fmt.Fprintf(canvas.Writer, `<svg x="%d" y="%d" width="%d" height="%d">\n`, x, y, itemSize, itemSize)
		{
			...
			canvas.Circle(config.AvatarSize/2, config.AvatarSize/2, config.AvatarSize/2, fmt.Sprintf(`stroke="#%s"`, config.BorderColor), fmt.Sprintf(`stroke-width="%d"`, config.Border), fmt.Sprintf(`fill="url(#%s)"`, id))
			canvas.Def()
			{
